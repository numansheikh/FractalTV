...
import { fetchTmdb, TmdbInvalidKeyError, setTmdbRateLimitListener as _setTmdbRateLimitListener } from './sources/tmdb'
...
// Session-scoped guards — prevent re-running augmentation on every detail open
// within the same process lifetime. Reset by restart or by force=true.
const tvmazeAugmentedThisSession = new Set<string>()
const tmdbAugmentedThisSession = new Set<string>()

// Signal raised when TMDB returns 401/403 — consumed by IPC layer to broadcast
// `enrichment:tmdb-invalid` and flip the `tmdb_key_invalid` setting.
export type TmdbInvalidKeyListener = () => void
let tmdbInvalidKeyListener: TmdbInvalidKeyListener | null = null
export function setTmdbInvalidKeyListener(fn: TmdbInvalidKeyListener | null) {
  tmdbInvalidKeyListener = fn
}
function handleTmdbInvalidKey() {
  try {
    setSetting('tmdb_key_invalid', '1')
    setSetting('enrichment_level', '1')
  } catch (e) {
    console.warn('[enrichment] failed to persist TMDB key-invalid flag:', e)
  }
  if (tmdbInvalidKeyListener) {
    try { tmdbInvalidKeyListener() } catch {}
  }
}

// Signal raised when TMDB rate-limit is hit after retries exhaust. IPC layer
// broadcasts `enrichment:tmdb-rate-limit` so the UI can show a pause banner.
export type TmdbRateLimitListener = (resumeAtMs: number) => void
export function setTmdbRateLimitListener(fn: TmdbRateLimitListener | null) {
  _setTmdbRateLimitListener(fn)
}
...
async function augmentWithTmdb(contentId: string, db: ReturnType<typeof getSqlite>, apiKey: string) {
  if (tmdbAugmentedThisSession.has(contentId)) return
  tmdbAugmentedThisSession.add(contentId)
  ...
  let tmdb: Awaited<ReturnType<typeof fetchTmdb>> = null
  try {
    tmdb = await fetchTmdb(imdbId, apiKey)
  } catch (e) {
    if (e instanceof TmdbInvalidKeyError) { handleTmdbInvalidKey(); return }
    throw e
  }
  if (!tmdb) return
...
async function augmentSeriesWithTvmaze(seriesId: string, db: ReturnType<typeof getSqlite>) {
  if (tvmazeAugmentedThisSession.has(seriesId)) return
  tvmazeAugmentedThisSession.add(seriesId)
...
export async function enrichForSource(
  ...
  if (force) {
    // Clear session guards so forced full-source enrichment can re-augment.
    for (const item of allItems) {
      tvmazeAugmentedThisSession.delete(item.id)
      tmdbAugmentedThisSession.delete(item.id)
    }
  }
  if (!force) {
    for (const item of allItems) {
  ...
  for (const item of toEnrich) {
    ...
    const tvmaze = enrichLevel === '1' && item.kind === 'series' && candidates.length > 0
      ...
      : null
    let tmdb: Awaited<ReturnType<typeof fetchTmdb>> = null
    if (enrichLevel === '2' && candidates.length > 0 && tmdbKey) {
      try {
        tmdb = await fetchTmdb(resolvedImdbId, tmdbKey)
      } catch (e) {
        if (e instanceof TmdbInvalidKeyError) { handleTmdbInvalidKey(); break }
        throw e
      }
    }
 ...
 */
export async function enrichSingle(contentId: string, force = false, maxLevel?: '0' | '1' | '2'): Promise<VodEnrichmentForContent> {
  const settingLevel = getEnrichmentLevel()
  const level: '0' | '1' | '2' = maxLevel && Number(maxLevel) < Number(settingLevel) ? maxLevel : settingLevel
  if (level === '0') return getForContent(contentId)
  ...
  if (force) {
    ...
    tvmazeAugmentedThisSession.delete(contentId)
    tmdbAugmentedThisSession.delete(contentId)
  } else {
  ...
  const tvmaze = level === '1' && !isMovie && candidates.length > 0
    ...
    : null
  let tmdb: Awaited<ReturnType<typeof fetchTmdb>> = null
  if (level === '2' && candidates.length > 0 && tmdbKey) {
    try {
      tmdb = await fetchTmdb(resolvedImdbId, tmdbKey)
    } catch (e) {
      if (e instanceof TmdbInvalidKeyError) { handleTmdbInvalidKey(); tmdb = null }
      else throw e
    }
  }
