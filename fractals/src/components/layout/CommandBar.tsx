...
const PLACEHOLDER_CYCLE = ['Search…', 'Try @ 2020', 'Try @ hindi 4k', 'Try @ year:2020'] as const
...
function CmdSearchInput({ query, setQuery, inputRef }: {
  ...
  // Cycle placeholder while input is idle — hint that `@` enables advanced search.
  useEffect(() => {
    if (isAdvanced || focused || visibleValue.length > 0) return
    const t = setInterval(() => {
      setPlaceholderIdx((i) => (i + 1) % PLACEHOLDER_CYCLE.length)
    }, PLACEHOLDER_CYCLE_MS)
    return () => clearInterval(t)
  }, [isAdvanced, focused, visibleValue])
  ...
  return (
    <div style={{ flex: 1, position: 'relative', height: 30, minWidth: 0 }}>
      ...
      <input
        ...
        placeholder={isAdvanced ? 'fr matrix 1999…' : PLACEHOLDER_CYCLE[placeholderIdx]}
        style={{
