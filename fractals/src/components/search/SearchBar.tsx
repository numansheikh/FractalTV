  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      if (meta && ['1', '2', '3', '4'].includes(e.key)) {
        e.preventDefault()
        setType(TYPES[parseInt(e.key) - 1].value)
      }
    }
  ...
  return (
    ...
    >
      ...
      <div
        ...
        onFocusCapture={(e) => { e.currentTarget.style.borderColor = 'rgba(124,77,255,0.45)' }}
        ...
        {TYPES.map((t) => {
          ...
          return (
            <button
              ...
              style={
                active
                  ? { background: 'var(--color-primary-dim)', color: 'var(--color-accent)', border: '1px solid rgba(124,77,255,0.25)' }
                  : { background: 'transparent', color: 'var(--color-text-secondary)', border: '1px solid transparent' }
              }
              onMouseEnter={(e) => { if (!active) { e.currentTarget.style.color = 'var(--color-text-primary)'; e.currentTarget.style.background = 'var(--color-card)' } }}
